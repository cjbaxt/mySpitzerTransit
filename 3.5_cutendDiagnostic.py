...
for m in range(len(PP)):
    ...
    if eclipse:
        coeffs_tuple_poly = ('per', 'rp', 'a', 'inc', 'ecc', 'w', 'u', 'limb_dark', 'fp', 't_secondary',
                       'K1', 'K2', 'K3', 'K4', 'K5',
                       'f', 'g', 'h')
    else:
    ...
    if eclipse:
        coeffs_tuple_PLD = ('per', 'rp', 'a', 'inc', 'ecc', 'w', 'u', 'limb_dark', 'fp', 't_secondary',
    ...
    lightcurve_red, timeseries_red, centroids_red, midtimes_red, background_red =  runPipeline(timeseries_badpix, midtimes,
           ...
           ramp_time = cutstarts[m/2], end_time = 0.,
           frametime = framtime,
    ...
    if method == 'poly':
        ...
        for n in range(ncuts):
            print '\nCutting {} minutes from end of lightcurve'.format(n*cuttime)
            ...
            if ncutframes == 0:
                lightcurve, timeseries, centroids, midtimes, background = lightcurve_red, timeseries_red, centroids_red, midtimes_red, background_red
            else:
            ...
            # Inflate the errors
            newlcerr = inflate_errs(popt, t, lc, lcerr, coeffs_dict_poly, coeffs_tuple_poly, fix_coeffs_poly, batman_params_poly, poly_params,x=x,y=y, method = method, eclipse = eclipse)
            ...
            if n == 0:
                plot_lightcurve(t,  lc, lcerr, popt, coeffs_dict_poly, coeffs_tuple_poly, fix_coeffs_poly, batman_params_poly, poly_params,
                                ...
                                method = "poly", color = 'b', scale = scale, filext = "cutEND", foldext=foldext,
                                showCuts = True, ncutstarts = ncuts, cutstartTime = cuttime, cutends = True, eclipse = eclipse)
        ...
        ind = np.where(np.array(cutstart_poly) == np.array(cutstart_poly).min())[0][0]
        ax[-1].plot(np.array(cutstart_poly)[ind]*(framtime/(60)), chi2s_poly[ind],  color='k', marker='o')
        ...
        plt.tight_layout()
        plt.savefig("{5}/PhD/SpitzerTransits/{0}{4}/{0}_{1}_{2}_{3}_ParamsVScutEndtime.png".format(planet, AOR, channel, 'poly', foldext, os.getenv('HOME')))
        plt.close()
    ...
    #PLD
        ...
        for n in range(ncuts):
            ...
            data = (t, Pns, lc, newlcerr, bounds, coeffs_dict_PLD, coeffs_tuple_PLD, fix_coeffs_PLD, batman_params_PLD, PLD_params, gaussian_priors, prior_coeffs, eclipse)
            ...
            if n == 0:
                plot_lightcurve(t,  lc, lcerr, popt_PLD, coeffs_dict_PLD, coeffs_tuple_PLD, fix_coeffs_PLD, batman_params_PLD, PLD_params,
                                ...
                                name = planet, channel = channel, orbit=AOR, savefile = True, TT_hjd = None,
                                method = "PLD", color = 'r', scale = scale, filext = "cutEnd", foldext=foldext,
                                showCuts = True, ncutstarts = ncuts, cutstartTime = cuttime, cutends = True, eclipse = eclipse)
        ...
        ind = np.where(np.array(cutstart_PLD) == np.array(cutstart_PLD).min())[0][0]
        ax[-1].plot(np.array(cutstart_PLD)[ind]*(framtime/(60)), chi2s_PLD[ind],  color='k', marker='o')
        ...
        plt.tight_layout()
        plt.savefig("{5}/PhD/SpitzerTransits/{0}{4}/{0}_{1}_{2}_{3}_ParamsVScutEndtime.png".format(planet, AOR, channel, 'PLD', foldext, os.getenv('HOME')))
        plt.close()
