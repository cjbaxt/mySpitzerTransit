...
for i in range(len(PP)):
    ...
    #     pass
        ...
        lightcurve, timeseries, centroids, midtimes, background, stats = runFullPipeline(datapath, 4, 30,
                       ...
                       quiet = False, plot = True, AOR = AOR, planet = planet, channel = channel, sysmethod = method, foldext=foldext, ret = True, binsize = binsize)

        stats['Cuttime'] = cutstart*60
        # Bin the lightcurves and timeseries (don't need to do as binning in function now)
        ...
        lc, lcerr = lightcurve, np.sqrt(lightcurve)
        ...
        if method == 'PLD':
            ...
            boxsize = PLDboxsize, gaussian_priors =gaussian_priors,
...
fitted_params = [key for key in labels_PLD if key in batman_params_PLD.__dict__]
